...
import { layoutShelfBooks, orderBooksByIds, hashSeed } from './shelfLayout'
...
import suppliedCover from './assets/dark-shelf/default-cover.png'
...
function useBookCover(book, customCover) {
  ...
  return { ref, cover: cover || suppliedCover }
}

function FaceBook({ book, customCover, progress, onOpen }) {
  const { ref, cover } = useBookCover(book, customCover)
  const percent = Math.round((progress || 0) * 100)
  return (
    <button ref={ref} className="v-face-book" onClick={() => onOpen(book)} title={book.title} aria-label={`${book.title}${percent ? `，阅读进度 ${percent}%` : ''}`}>
      <img src={cover} alt="" />
...
function SpineBook({ book, customCover, progress, onOpen }) {
  const { ref, cover } = useBookCover(book, customCover)
  const seed = hashSeed(book.id || book.path || book.title)
  const percent = Math.round((progress || 0) * 100)
  return (
    <button ref={ref} className={`v-spine v-spine-${seed % SPINES.length}`} onClick={() => onOpen(book)} title={book.title} aria-label={`${book.title}${percent ? `，阅读进度 ${percent}%` : ''}`}>
      <img className="v-spine-art" src={SPINES[seed % SPINES.length]} alt="" />
      <span className="v-spine-reveal" aria-hidden="true">
        <span className="v-spine-reveal-meta"><strong>{book.title}</strong><small>阅读进度 <i style={{ '--progress': `${percent}%` }} />{percent}%</small></span>
        <img src={cover} alt="" />
      </span>
    </button>
  )
...
function ShelfRow({ row, progressMap, coversMap, onOpen, draggingCategory, dropState, onDragStart, onDragOver, onDrop, onDragEnd, onMoveByKeyboard, onOpenCategory }) {
  ...
  useLayoutEffect(() => {
    ...
    const update = () => setWidth(Math.max(280, node.clientWidth - (row.decorated ? 94 : 16)))
    update()
  ...
  return (
    <section className={`v-shelf-row ${row.decorated ? 'is-decorated' : ''} ${draggingCategory === row.key ? 'is-dragging' : ''} ${dropClass}`} onDragOver={canDrag ? (event) => onDragOver(event, row.key) : undefined} onDrop={canDrag ? (event) => onDrop(event, row.key) : undefined}>
      ...
      <div ref={booksRef} className="v-shelf-books" style={{ '--cover-width': `${layout.coverWidth}px`, '--cover-height': `${Math.round(layout.coverWidth * 1.333)}px` }}>
        {row.decorated ? <img className="v-shelf-moon" src={shelfMoon} alt="" /> : null}
        <div className="v-face-group">
          {layout.covers.map((book) => <FaceBook key={book.id} book={book} customCover={coversMap[book.id]} progress={progressMap[book.id]?.percent} onOpen={onOpen} />)}
        </div>
        {layout.spines.length ? <div className="v-spine-group">{layout.spines.map(({ book }) => <SpineBook key={book.id} book={book} customCover={coversMap[book.id]} progress={progressMap[book.id]?.percent} onOpen={onOpen} />)}</div> : null}
      </div>
...
function LibraryTopBar({ bookCount, query, onQueryChange }) {
  return (
...
export default function VirtualBookshelfHome({ books, progressMap, statusMap, coversMap, categories, tagsMap, categoryBookOrder, recentBookIds, onOpen, onAddBooks, onOpenLibrary, onOpenNotes, onOpenAppearance, onReorderCategories, onToggleTheme, scrollMemory }) {
  ...
  const rows = useMemo(() => {
    ...
    const readingAll = books.filter((book) => getStatus(book, progressMap, statusMap) === 'reading')
    const reading = readingAll.filter(matches)
    if (reading.length) result.push({ key: 'reading', label: '在读', managementKey: 'reading', books: reading, totalCount: readingAll.length })
    const finishedAll = books.filter((book) => getStatus(book, progressMap, statusMap) === 'finished')
    const finished = finishedAll.filter(matches)
    if (finished.length) result.push({ key: 'finished', label: '读完', managementKey: 'finished', books: finished, totalCount: finishedAll.length })
    for (const category of categories) {
      ...
      if (visible.length) result.push({ key: category, label: category, managementKey: 'category', books: visible, totalCount: members.length, reorderable: true })
    }
    return result
  ...
  return (
    <main className="v-home" aria-label="书脊视图">
      <LibraryTopBar bookCount={books.length} query={query} onQueryChange={setQuery} />
      <div className="v-bookshelf-scene" ref={sceneRef}>
        {rows.length ? <div className="v-shelf-stack">{rows.map((row) => <ShelfRow key={row.key} row={row} progressMap={progressMap} coversMap={coversMap} onOpen={onOpen} draggingCategory={draggingCategory} dropState={dropState} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop} onDragEnd={() => { setDraggingCategory(''); setDropState(null) }} onMoveByKeyboard={moveByKeyboard} onOpenCategory={onOpenLibrary} />)}</div> : query ? <div className="v-empty-shelf"><strong>没有找到相关书籍</strong></div> : <div className="v-empty-shelf"><strong>书架还是空的</strong><button onClick={onAddBooks}>导入书籍</button></div>}
      </div>
