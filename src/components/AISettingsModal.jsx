        ...
        {!settings ? <div className="ai-loading"><RefreshCw className="spin" size={20} /> 正在读取本地设置</div> : (
          <div className="ai-provider-layout">
            ...
            <form className="ai-provider-form" onSubmit={saveProvider}>
              ...
              <div className="ai-security-note"><ShieldCheck size={15} /><span>{settings.encryptionAvailable ? `Key 使用${settings.secureStorageLabel || '系统安全存储'}加密，页面无法读回。` : '系统安全存储当前不可用，将拒绝保存 Key。'}</span></div>
