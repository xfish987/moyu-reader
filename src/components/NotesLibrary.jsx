...
// 卡片摘录：有高亮时以高亮为中心截取窗口，上下文淡显（仔细看得到上一行/下一行）；
// 无高亮时从头显示，交给 CSS line-clamp 截断。
function QuoteExcerpt({ note }) {
  ...
  const segments = segmentByHighlights(note.text, highlights, start, end)
  return (
    <blockquote>
...
export default function NotesLibrary({ books, bookMetadata, notesMap, appearanceTheme, onOpenNote, onAddNote, onUpdateNote, onDeleteNote, onCreateGroup, onExportNotes }) {
              ...
              {visible.length ? (
                <div className={`quote-grid ${viewMode === 'list' ? 'is-list' : ''}`}>
                  {[...visible].sort((a, b) => b.createdAt - a.createdAt).map((note) => (
                    <article className="quote-card" key={note.id} onClick={() => setDetailTarget({ note, book })}>
                      ...
                      <footer>
                        <span className="quote-source">{note.source || `《${book.title}》`} · {new Date(note.createdAt).toLocaleDateString('zh-CN')}</span>
                        <div onClick={(event) => event.stopPropagation()}>
                          ...
                          {!note.custom && !book.missing ? <button onClick={() => onOpenNote(book, note)} title="跳转到原文"><ArrowUpRight size={16} /></button> : null}
                        </div>
                      </footer>
                    </article>
                  ))}
