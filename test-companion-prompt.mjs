...
assert.ok(summary[0].content.includes('剧情梳理助手'))
assert.ok(summary[0].content.includes('timePoint'), 'system 应包含 JSON schema')
assert.ok(summary[0].content.includes('严禁') || summary[0].content.includes('不剧透'), 'system 应包含防剧透约束')
assert.ok(summary[0].content.includes('闭合'), 'system 应要求闭合 JSON')
assert.ok(summary[1].content.includes('第十八章 归来'), 'user 应包含章节标签')
assert.ok(summary[1].content.includes('他推开城门'), 'user 应包含章节文本')
assert.ok(summary[1].content.includes('第9-17章尚未总结'), 'user 应包含 coverageNote')
assert.ok(summary[1].content.includes('第七章 远行'), 'user 应包含前序总结')
...
assert.ok(!bare[1].content.includes('覆盖范围说明'), '无 coverageNote 时不应出现该段落')
...
assert.ok(chat[0].content.includes('严禁剧透'), 'system 应包含不剧透约束')
assert.ok(chat[0].content.includes('还没有相关信息'), 'system 应包含缺材料的兜底话术')
assert.ok(chat[0].content.includes('简体中文'))
assert.ok(chat[0].content.includes('第一章 开端'), 'system 应包含剧情梳理')
assert.ok(chat[0].content.includes('青牛村'), '剧情梳理应包含地点')
assert.ok(chat[0].content.includes('方运'), 'system 应包含设定集')
assert.ok(chat[0].content.includes('青云剑'))
// 结构：system + history(有效2条) + user(question)
assert.equal(chat.length, 4)
assert.deepEqual(chat[1], { role: 'user', content: '主角为什么离开村子？' })
assert.deepEqual(chat[2], { role: 'assistant', content: '因为村子被毁。' })
assert.equal(chat[3].role, 'user')
assert.equal(chat[3].content, '青云剑现在还在主角手上吗？')

// 空兜底不应抛错
const emptyChat = buildCompanionChatMessages({ question: 'q' })
assert.equal(emptyChat.length, 2)
assert.equal(emptyChat[1].content, 'q')
assert.ok(emptyChat[0].content.includes('还没有剧情梳理'))
assert.ok(emptyChat[0].content.includes('还没有设定集资料'))
