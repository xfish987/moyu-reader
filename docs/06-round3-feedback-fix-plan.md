...
### F6 中等窗口（681–900px）分类侧栏挤压变形
- 根因：旧基线在 ≤900px 把分类导航改成横向 flex chips，但 UI B 在该带宽仍给侧栏留 140px 窄列，chips 在窄列里横滚，标题与筛选行叠压。
- 修复：≤900px 起 `.shelf-layout` 改单列，分类导航整行横排 chips（含内联「＋」新建按钮），标题隐藏；≤680px 的重复规则删除，统一由 ≤900 块承担。

### F7 AI 设置 / 快捷键等旧弹窗不跟随主题
- 根因：这些弹窗用 styles.css 的老 Token（--paper/--surface/--ink…），UI B 主题只定义 b- 变量，弹窗永远是默认浅色。
- 修复：在 `.ui-b.ui-b-theme-*` 上把老 Token 映射到 b- 变量（含 accent/danger 系列），选择器优先级压过阅读态 `.app-shell.theme-*`；阅读区自身 Token 定义在更近的 `.reader-view.theme-*` 上，不受影响。

### F8 阅读纸页与封面去外框
- 文字/EPUB 纸页去描边圆角，消除双层框；有封面图不再垫彩色相框。
- 空书架不再画装饰层板，架子只随书籍出现。
